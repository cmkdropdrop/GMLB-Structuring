...
def source_hash(root: Path) -> str:
    h = hashlib.sha256()
    for path in sorted((root / "policy_engine").glob("*.py")):
...
**Run:** Engine {policy_engine.__version__}, Heston-Hull-White unter Q / Black-Scholes-Hull-White unter Real World, {args.base_paths:,} Basispfade, Seed 2026; erstellt {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}.  
...
def main() -> None:
    ...
    manifest = {
        ...
        "engine_version": policy_engine.__version__,
