...
the complete Dynamic and deployed LSMC policies for the first model point of the
...
DEFAULT_MARKET_CACHE_ROOT = SCRIPT_DIRECTORY / "cache" / "q_market_paths"
DEFAULT_HEDGE_CACHE_ROOT = SCRIPT_DIRECTORY / "cache" / "q_hedge_prices"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "deprecated compatibility option; the ordered annual LSMC permits "
            "only CONTINUE_FOR_ONE_YEAR or FULL_WITHDRAWAL_NOW"
        ),
    )
    parser.add_argument(
        "--lsmc-ridge",
    ...
    if args.lsmc_income_action_set != "continue_full":
        parser.error(
            "--lsmc-income-action-set=continue_partial_full is incompatible "
            "with the ordered annual optimal-behaviour policy"
        )
...
def _estimate_horizon_months(
    ...
    """Resolve the longest default projection from relevant covered lives."""
    path = model_points_path.expanduser().resolve()
    minimum_age: Optional[float] = None
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            for row in csv.DictReader(handle):
                relevant_fields = ["primary_age"]
                has_continuing_spouse = (
                    str(row.get("spouse", "")).strip().lower()
                    in {"1", "true", "yes", "y"}
                    and str(row.get("spouse_death_election", "")).strip()
                    == "continue_income"
                )
                if has_continuing_spouse:
                    relevant_fields.append("secondary_age")
                for field in relevant_fields:
                    ...
                    if math.isfinite(age) and 0.0 < age < DEFAULT_PROJECTION_MAX_AGE:
                        ...
                        raise ValueError(f"invalid required {field}")
...
def _run_cache_precompute_jobs(
    ...
    *,
    ...
    if not jobs:
        log_to_console(
            "CACHE-VORPHASE | Keine neuen Bewertungsläufe; keine "
            "Cache-Prüfung erforderlich."
        )
        return
    phase_started = time.perf_counter()
    log_to_console(
        f"CACHE-VORPHASE START | {len(jobs)} exakte Precompute-Aufrufe | "
        "vorhandene Einträge werden validiert, fehlende erzeugt."
    )
    for index, job in enumerate(jobs, start=1):
        ...
        log_to_console(
            f"CACHE {index}/{len(jobs)} FERTIG | {job.label} | Laufzeit "
            f"{_format_duration(time.perf_counter() - job_started)}"
        )
    log_to_console(
        f"CACHE-VORPHASE FERTIG | {len(jobs)} Aufrufe | Laufzeit "
...
def _execute_scenario_job(job: ScenarioJob, blas_threads: int) -> None:
    ...
    log_to_console(
        f"START 2/2 | {job.label} | LSMC V11-Kandidat: Training, Validierung, "
    ...
    log_to_console(
        f"FERTIG 2/2 | {job.label} | LSMC-Deployment | Laufzeit "
...
def _load_scenario_result(
    ...
    *,
    ...
    row: dict[str, object] = {
        ...
        "every_training_seed_passes_validation": _as_bool(
            validation_settings.get(
                "every_seed_passes_or_deploys_validated_fallback"
            )
        ),
        "every_training_seed_candidate_passes_election_income_combined": (
            _as_bool(validation_settings.get(
                "every_seed_passes_election_income_combined"
            ))
        ),
        "every_training_seed_passes_or_deploys_validated_fallback": (
            _as_bool(validation_settings.get(
                "every_seed_passes_or_deploys_validated_fallback"
            ))
        ),
        ...
        "lsmc_deployed_policy_by_training_seed_json": json.dumps(
            validation_settings.get("deployed_policy_by_training_seed", {}),
            sort_keys=True,
        ),
...
def _create_plots(
    ...
    labels = {
        ...
        "lsmc": "Fitted annual-action LSMC lower bound",
    }
...
def _write_report(
    ...
    lines = [
        ...
        (
            ...
            "CSV-konfigurierten Referenzfondsreturn: "
            "`min(max(R_fund, 0), Cap)`. Nur 6 % "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    log_to_console(
        "BEHAVIOUR | Pro Szenario nur Dynamic V11 und validiertes "
        "LSMC-Deployment; "
    ...
    log_to_console(
        ...
        f"{len(cache_precompute_jobs)} Cache-Precompute-Aufrufe, "
    ...
    manifest = {
        ...
        "method": {
            ...
            "behaviour_arms_retained": {
                ...
                "lsmc_v11": (
                    "validated V11 candidate or recorded fixed validation "
                    "fallback"
                ),
            },
        ...
        "runner_validation": {
            ...
            "every_training_seed_passes_election_income_combined": all(
                _as_bool(row[
                    "every_training_seed_candidate_passes_election_income_combined"
                ])
                for row in all_fit_rows
            ),
            "every_training_seed_passes_or_deploys_validated_fallback": all(
                _as_bool(row[
                    "every_training_seed_passes_or_deploys_validated_fallback"
                ])
                for row in all_fit_rows
