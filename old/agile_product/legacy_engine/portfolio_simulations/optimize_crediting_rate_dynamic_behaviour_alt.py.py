...
# Bellman step, preserving the existing RegressionPolicyYear interface.  The
...
SLIM_REWARD_TRANSITION_BASIS_DIMENSION = 6
...
MAXIMUM_DEPLOYMENT_ARGMAX_MISMATCH_FRACTION = 0.20
...
def _slim_reward_transition_design(
    raw_state: Array,
    state_feature_names: Sequence[str],
    mean: Array | None = None,
    scale: Array | None = None,
    *,
    ...
    """Build ``[1, z(C_atm), z(S), z(r), z(A), z(A)^2]``.
    ...
    if (mean is None) != (scale is None):
        raise ValueError("Reward/transition mean and scale must be supplied together.")
    if mean is None or scale is None:
        mean, scale = _raw_scaling(factors)
    linear = _standardised_linear_design(factors, mean, scale)
    design = np.column_stack((linear, linear[:, 4] ** 2))
    if design.shape[1] != SLIM_REWARD_TRANSITION_BASIS_DIMENSION:
        raise RuntimeError("Slim reward/transition basis has an invalid width.")
    return design, np.asarray(mean, dtype=float), np.asarray(scale, dtype=float)
...
def _backward_induction(
    ...
    *,
    ...
    ``y + 1`` it is stored as one regression on the slim exogenous basis
    ``[1, C_atm, S, r]`` per grid node (Boogert & de Jong: a separate
    ...
    up at the *resulting* account value.  Reward and account-value transition
    use the six-column basis ``[1, C_atm, S, r, A, A^2]``, with all economic
    factors standardised and ``A^2`` defined as the square of standardised
    inventory.  The transition per cap is empirical because crediting with
    fees, mortality, dynamic income and withdrawals is not closed-form, unlike
    the analytic gas ``v -> v + dv`` transition.  The continuation is the grid
    value function **interpolated** at that resulting level.  The interpolation
    uses the delta between neighbouring nodes ``V(A_{k+1}) - V(A_k)`` -- the
    discrete marginal value of account value (the storage shadow price
    ``dV/dA``) -- which is what makes the continuation sensitive to where the
    fund lands and therefore what drives the optimal cap.  Piecewise-linear
    interpolation with boundary clamping keeps the continuation bounded over
    ...
    first-cap choice is made on a selection pool; its reported value uses the
    direct realised-action estimator on fold zero, which is excluded from every
    regression and grid fit in the recursion.
    ...
    if folds < 2:
        raise ValueError("Backward induction needs at least two honest folds.")
    ...
    if len(basis_names_full) != 25:
        raise RuntimeError(
            "Deployable cap policy must retain the full 25-column basis."
        )
    minimum_training_count = max(
        30, 3 * SLIM_REWARD_TRANSITION_BASIS_DIMENSION
    )
    n_actions = len(ACTION_CAPS)

    # Fold zero is an honest evaluation pool that is never used by any
    # regression, scaling estimate, inventory-grid construction or Bellman
    # maximisation fit.  Stratification by the first action guarantees direct
    # year-zero coverage for every cap.  Remaining folds form the selection
    # pool and are reused only for within-selection OOF diagnostics.
    rng = np.random.default_rng(seed)
    fold_ids = np.empty(n_paths, dtype=np.int64)
    for action in range(n_actions):
        rows = np.flatnonzero(action_indices[:, 0] == action)
        if rows.size < folds:
            raise ValueError(
                f"First-year cap {ACTION_CAPS[action]:.4%} has only "
                f"{rows.size} paths for {folds} honest folds."
            )
        assigned = np.resize(np.arange(folds, dtype=np.int64), rows.size)
        fold_ids[rows] = rng.permutation(assigned)
    holdout_pool = fold_ids == 0
    selection_pool = ~holdout_pool
    if not np.any(holdout_pool) or not np.any(selection_pool):
        raise RuntimeError("Honest selection/holdout split is empty.")
    ...
    pooled_av = np.concatenate([
        account_value[selection_pool & (exposure[:, year] > 0.0), year]
    ...
    for year in range(n_years - 1, -1, -1):
        ...
        _, continuation_mean, continuation_scale = _slim_continuation_design(
            raw_year[selection_pool], feature_names
        )
        ...
        _, rt_mean, rt_scale = _slim_reward_transition_design(
            raw_year[selection_pool], feature_names
        )
        design_rt, _, _ = _slim_reward_transition_design(
            raw_year, feature_names, rt_mean, rt_scale
        )
        mean_full, scale_full = _raw_scaling(raw_year[selection_pool])
        deploy_design = _design_matrix(
            raw_year, mean_full, scale_full, nonlinear_full, interactions_full
        )
        if deploy_design.shape[1] != 25:
            raise RuntimeError("Deployable policy design must have 25 columns.")
        has_continuation = (year + 1) in active_set

        # Grid value function at the realised next-year exogenous state: this is
        # the direct sample used by the honest fold-zero year-0 estimator and
        # the basis for the conditional expectation used by later Bellman steps.
        if has_continuation:
        ...
        reward_beta = np.zeros(
            (n_actions, SLIM_REWARD_TRANSITION_BASIS_DIMENSION, 9)
        )
        transition_beta = np.zeros(
            (n_actions, SLIM_REWARD_TRANSITION_BASIS_DIMENSION)
        )
        reward_lower_bounds = np.zeros((n_actions, 10))
        reward_upper_bounds = np.zeros((n_actions, 10))
        deployable = np.ones(n_actions, dtype=bool)
        action_mask_reasons: list[list[str]] = [
            [] for _ in range(n_actions)
        ]
        action_counts = np.zeros(n_actions, dtype=np.int64)
        holdout_action_counts = np.zeros(n_actions, dtype=np.int64)
        action_condition_numbers = np.zeros(n_actions)
        for action in range(n_actions):
            selected = selection_pool & (observed == action)
            count = int(np.count_nonzero(selected))
            action_counts[action] = count
            holdout_action_counts[action] = int(np.count_nonzero(
                holdout_pool & (observed == action)
            ))
            if count < minimum_training_count:
                deployable[action] = False
                reason = f"support_{count}_below_{minimum_training_count}"
                action_mask_reasons[action].append(reason)
                numerical_reasons.add(
            ...
            design_sel = design_rt[selected]
            condition = _condition_number(design_sel, ridge)
            action_condition_numbers[action] = condition
            if year > 0 and (
                not np.isfinite(condition)
                or condition > MAXIMUM_ACTION_DESIGN_CONDITION_NUMBER
            ):
                reason = f"condition_{condition:.6g}_above_1e10"
                action_mask_reasons[action].append(reason)
                numerical_reasons.add(
                    f"policy_year_{year + 1}::cap_{ACTION_CAPS[action]:.6f}_"
                    f"{reason}"
                )
                # With adequate support, the contractual minimum cap remains
                # the one physical fallback required by the policy interface.
                if action != 0:
                    deployable[action] = False
            ...
            (
                reward_lower_bounds[action],
                reward_upper_bounds[action],
            ) = _component_prediction_bounds(immediate[selected, year, :])
            if has_continuation:
        ...
        if not np.any(deployable):
            raise ValueError(
                f"No cap has the minimum {minimum_training_count} training "
                f"paths in policy year {year + 1}. Increase --n-paths."
            )

        if year == 0:
            # Select the first cap only on the selection pool.  Report its
            # value on fold zero, which was excluded from every regression,
            # scaling estimate and inventory-grid fit in the recursion.
            ...
            selection_action_values = np.zeros((n_actions, 10))
            holdout_action_values = np.zeros((n_actions, 10))
            selection_action_standard_errors = np.zeros(n_actions)
            holdout_action_standard_errors = np.zeros(n_actions)
            for action in range(n_actions):
                selection = selection_pool & (observed == action)
                holdout = holdout_pool & (observed == action)
                if not np.any(selection) or not np.any(holdout):
                    raise RuntimeError(
                        "Honest first-year coverage missing for cap "
                        f"{ACTION_CAPS[action]}."
                    )
                selection_components = np.mean(
                    realised_value[selection], axis=0
                )
                holdout_components = np.mean(realised_value[holdout], axis=0)
                selection_action_values[action] = _with_derived_csm(
                    selection_components[None, :]
                )[0]
                holdout_action_values[action] = _with_derived_csm(
                    holdout_components[None, :]
                )[0]
                selection_action_standard_errors[action] = _standard_error(
                    realised_csm[selection]
                )
                holdout_action_standard_errors[action] = _standard_error(
                    realised_csm[holdout]
                )
            chosen_action = int(_masked_lower_cap_argmax(
                selection_action_values[:, 0], deployable, axis=0
            ))
            first_cap = float(ACTION_CAPS[chosen_action])
            first_outputs = holdout_action_values[chosen_action]
            first_se = float(holdout_action_standard_errors[chosen_action])
            LOGGER.info(
                "First-year honest gas-storage diagnostic | cap %.2f%% | "
                "selection Q-CSM %.2f | holdout Q-CSM %.2f | holdout SE %.2f",
                100.0 * first_cap,
                selection_action_values[chosen_action, 0],
                first_outputs[0],
                first_se,
            )
            for action, cap in enumerate(ACTION_CAPS):
                selection_values = selection_action_values[action]
                holdout_values = holdout_action_values[action]
                first_year_action_rows.append({
                    "cap": float(cap),
                    "cap_percent": 100.0 * float(cap),
                    "selection_estimated_q_new_business_csm_proxy": float(
                    ...
                    "selection_standard_error_new_business_csm_proxy": float(
                        selection_action_standard_errors[action]
                    ),
                    "holdout_estimated_q_new_business_csm_proxy": float(
                        holdout_values[0]
                    ),
                    "holdout_estimated_q_fees_product": float(holdout_values[1]),
                    "holdout_estimated_q_fees_lip": float(holdout_values[2]),
                    "holdout_estimated_q_crediting_margin": float(
                        holdout_values[3]
                    ),
                    "holdout_estimated_q_mva_retained": float(holdout_values[4]),
                    "holdout_estimated_q_aps_retained": float(holdout_values[5]),
                    "holdout_estimated_q_guarantee_claims": float(
                        holdout_values[6]
                    ),
                    "holdout_estimated_q_other_insurer_funded_benefits": float(
                    ...
                    "holdout_estimated_q_expenses": float(holdout_values[8]),
                    "holdout_estimated_q_hedge_costs": float(holdout_values[9]),
                    "holdout_standard_error_new_business_csm_proxy": float(
                        holdout_action_standard_errors[action]
                    ),
                    "selection_path_count": int(action_counts[action]),
                    "holdout_path_count": int(holdout_action_counts[action]),
                    "observed_control_action_path_count": int(
                        action_counts[action] + holdout_action_counts[action]
                    ),
                    "selection_value_semantics": (
                    ...
                    "holdout_value_semantics": (
                        "fold-zero direct realised-action target; fold zero is "
                        "excluded from every fitted continuation and scaling"
                    ),
                    "economically_active": True,
                    "is_optimal_first_year_cap": action == chosen_action,
                    "outer_fold_pure": False,
                    "honest_fold_zero_excluded_from_fit": True,
                    "csm_derived_from_components": True,
                })
            first_policy = _constant_first_year_policy(
                year=0,
                raw_state=raw_states[:, 0, :],
                action_values=selection_action_values,
                action_standard_errors=selection_action_standard_errors,
                feature_names=feature_names,
                numerically_stable=(
                    bool(np.all(deployable))
                    and not any(action_mask_reasons)
                ),
            )
            first_policy = replace(
                first_policy,
                action_deployable_mask=deployable.copy(),
            )
        ...
        if has_continuation:
            conditional_beta = _ridge_fit_multioutput(
                continuation_design[selection_pool],
                node_next_realised[selection_pool].reshape(
                    int(np.count_nonzero(selection_pool)), n_nodes * 9
                ),
                ridge,
            )
            node_next = (continuation_design @ conditional_beta).reshape(
                n_paths, n_nodes, 9
            )
            node_next, _ = _clip_component_tensor_by_cell(
                node_next,
                year_values[year + 1].node_lower_bounds,
                year_values[year + 1].node_upper_bounds,
            )
        else:
            node_next = np.zeros((n_paths, n_nodes, 9))

        # Per-cap Q at each path's own inventory is available before the grid
        # maximum.  Use its honest realised-action residuals and the deployment
        # projection fidelity to finish the action mask before any Bellman
        # maximisation can exploit an unreliable fit.
        reward_own = np.einsum("pb,abo->pao", design_rt, reward_beta)
        reward_own, own_reward_clip_fraction = _clip_component_tensor_by_cell(
            reward_own, reward_lower_bounds, reward_upper_bounds
        )
        if has_continuation:
            next_level_own = np.einsum("pb,ab->pa", design_rt, transition_beta)
        ...
        realised_target = immediate[:, year, 1:].copy()
        if has_continuation:
            realised_continuation, _ = _grid_continuation_lookup(
                node_next_realised, grid, account_value[:, year + 1]
            )
            realised_target = realised_target + realised_continuation
        realised_target_csm = _csm_from_component_values(realised_target)

        action_oof_rmse = np.zeros(n_actions)
        action_oof_r_squared = np.zeros(n_actions)
        for action in range(n_actions):
            selected = selection_pool & (observed == action)
            oof_rmse, oof_r_squared = _kfold_oof_csm_diagnostics(
                design_rt[selected],
                realised_target_csm[selected],
                folds,
                ridge,
                seed + action + 101 * year,
            )
            action_oof_rmse[action] = oof_rmse
            action_oof_r_squared[action] = oof_r_squared
            if not np.isfinite(oof_r_squared) or (
                oof_r_squared < MINIMUM_ACTION_OOF_R_SQUARED
            ):
                reason = f"oof_r_squared_{oof_r_squared:.6g}_below_zero"
                action_mask_reasons[action].append(reason)
                numerical_reasons.add(
                    f"policy_year_{year + 1}::cap_{ACTION_CAPS[action]:.6f}_"
                    f"{reason}"
        ...
        q_flat = q_own[selection_pool].reshape(
            int(np.count_nonzero(selection_pool)), n_actions * 9
        )
        beta_flat = _ridge_fit_multioutput(
            deploy_design[selection_pool], q_flat, ridge
        )
        deploy_component_beta = np.transpose(
            beta_flat.reshape(deploy_design.shape[1], n_actions, 9), (1, 0, 2)
        )  # (n_actions, 25, 9)
        ...
        deploy_lower_bounds = np.zeros((n_actions, 10))
        deploy_upper_bounds = np.zeros((n_actions, 10))
        for action in range(n_actions):
            (
                deploy_lower_bounds[action],
                deploy_upper_bounds[action],
            ) = _component_prediction_bounds(
                _with_derived_csm(q_own[selection_pool, action, :])
            )

        deploy_components = np.einsum(
            "pb,abo->pao", deploy_design, deploy_component_beta
        )
        deploy_components, _ = _clip_component_tensor_by_cell(
            deploy_components, deploy_lower_bounds, deploy_upper_bounds
        )
        deploy_csm = _csm_from_component_values(deploy_components)
        slim_choice = _masked_lower_cap_argmax(
            csm_own[selection_pool], deployable, axis=1
        )
        projected_choice = _masked_lower_cap_argmax(
            deploy_csm[selection_pool], deployable, axis=1
        )
        projection_choice_mismatch = float(np.mean(
            slim_choice != projected_choice
        ))
        if (
            projection_choice_mismatch
            > MAXIMUM_DEPLOYMENT_ARGMAX_MISMATCH_FRACTION
        ):
            reason = (
                "deploy_argmax_mismatch_"
                f"{projection_choice_mismatch:.6f}_above_"
                f"{MAXIMUM_DEPLOYMENT_ARGMAX_MISMATCH_FRACTION:.2f}"
            )
            numerical_reasons.add(f"policy_year_{year + 1}::{reason}")
            for action in range(1, n_actions):
                deployable[action] = False
                action_mask_reasons[action].append(reason)

        # Evaluate all forced inventory nodes first, then apply extrapolation
        # masks once for the complete year before selecting any action.
        node_q = np.zeros((n_paths, n_nodes, n_actions, 9))
        maximum_node_reward_clip_fraction = np.zeros(n_actions)
        for node, level in enumerate(grid):
            design_node, _, _ = _slim_reward_transition_design(
                raw_year,
                feature_names,
                rt_mean,
                rt_scale,
                inventory_level=float(level),
            )
            reward_node = np.einsum("pb,abo->pao", design_node, reward_beta)
            reward_node, reward_clip_fraction = _clip_component_tensor_by_cell(
                reward_node, reward_lower_bounds, reward_upper_bounds
            )
            maximum_node_reward_clip_fraction = np.maximum(
                maximum_node_reward_clip_fraction, reward_clip_fraction
            )
            if has_continuation:
            ...
            node_q[:, node, :, :] = reward_node + continuation_node

        for action in range(1, n_actions):
            clip_fraction = maximum_node_reward_clip_fraction[action]
            if clip_fraction > MAXIMUM_COUNTERFACTUAL_COMPONENT_CLIP_FRACTION:
                reason = f"counterfactual_clip_{clip_fraction:.6f}_above_0.20"
                deployable[action] = False
                action_mask_reasons[action].append(reason)
                numerical_reasons.add(
                    f"policy_year_{year + 1}::cap_{ACTION_CAPS[action]:.6f}_"
        ...
        if not np.any(deployable):
            raise RuntimeError(
                f"Policy year {year + 1} has no executable cap action."
            )

        node_value_target = np.zeros((n_paths, n_nodes, 9))
        for node in range(n_nodes):
            csm_node = _csm_from_component_values(node_q[:, node, :, :])
            if not np.all(np.isfinite(csm_node)):
                raise RuntimeError(
                    f"Non-finite node Q values at policy year {year + 1}."
                )
            best = _masked_lower_cap_argmax(csm_node, deployable, axis=1)
            node_value_target[:, node, :] = np.take_along_axis(
                node_q[:, node, :, :], best[:, None, None], axis=1
            )[:, 0, :]

        year_values[year] = _fit_grid_continuation(
            continuation_design[selection_pool],
            node_value_target[selection_pool],
            continuation_mean,
            continuation_scale,
            ridge,
        )
        node_csm_curve = _csm_from_component_values(np.mean(
            node_value_target[selection_pool], axis=0
        ))
        marginal_value = np.diff(node_csm_curve) / np.maximum(
            np.diff(grid), 1.0e-12
        )
        LOGGER.debug(
        ...
        for action, cap in enumerate(ACTION_CAPS):
            selected = selection_pool & (observed == action)
            target_csm = realised_target_csm[selected]
            fitted_csm = deploy_csm[selected, action]
            ...
            regression_rows.append({
                ...
                "basis_dimension": int(design_rt.shape[1]),
                "ridge_multiplier": float(ridge),
                "out_of_fold_rmse_new_business_csm_proxy": float(
                    action_oof_rmse[action]
                ),
                "out_of_fold_r_squared_new_business_csm_proxy": float(
                    action_oof_r_squared[action]
                ),
                "in_sample_component_clip_fraction": float(
                    own_reward_clip_fraction[action]
                ),
                "maximum_fold_component_clip_fraction": 0.0,
                "counterfactual_component_clip_fraction": float(
                    maximum_node_reward_clip_fraction[action]
                ),
                "maximum_fold_counterfactual_component_clip_fraction": float(
                    maximum_node_reward_clip_fraction[action]
                ),
                "deployment_argmax_mismatch_fraction": float(
                    projection_choice_mismatch
                ),
                ...
                "regularized_normal_matrix_condition_number": float(
                    action_condition_numbers[action]
                ),
                "design_condition_number": float(
                    action_condition_numbers[action]
                ),
        ...
        policy_years.append(RegressionPolicyYear(
            ...
            value_lower_bounds=deploy_lower_bounds,
            value_upper_bounds=deploy_upper_bounds,
            numerically_stable=(
                bool(np.all(deployable))
                and not any(action_mask_reasons)
            ),
...
def _policy_payload(
    ...
    *,
    ...
    return {
        ...
        "method": "honest_fold_zero_gas_storage_lsmc",
        ...
        "numerically_stable_semantics": (
            ...
            "counterfactual-extrapolation diagnostics; failed non-minimum "
            "actions are excluded locally while the contractual minimum cap "
            "is retained solely as a physical recursion fallback"
        ),
...
def _plot_flexibility_value(
    *,
    ...
    bottom.set_xticks(
        [0.0], ["Validiert deployed minus bester fixer Cap"]
    )
    ...
    if np.isclose(delta_se, 0.0) and np.isclose(delta, 0.0):
        ...
        evidence = "validierte Deployment-Policy entspricht dem fixen Cap"
    else:
        evidence = (
    ...
    bottom.set_title(
        f"Validierter Flexibilitätswert ggü. fairem fixer Vergleich: "
    ...
    figure.text(
        0.01, 0.01,
        "Alle Balken stammen aus direkten Monatsprojektionen. Der primäre "
        "Flexibilitätswert gehört zur validierten Deployment-Policy; ein "
        "abgelehnter Research-Kandidat bleibt nur als Diagnose sichtbar. Das "
        "Δ-Intervall nutzt gepaarte "
...
def _plot_first_year_choice(
    *,
    ...
    has_honest_fold_zero = all(
        int(row.get("holdout_path_count", 0)) > 0 for row in ordered
    )
    ...
    axis.errorbar(
        ...
        label=(
            ...
            else "Complete-path outer-fold-pure OOF Q"
            if outer_fold_pure
    ...
    axis.axvline(
        ...
        label=f"Masked selection-pool Bellman choice: {chosen_cap:g}%",
    )
    axis.axvline(
    ...
    axis.set_title(
        "First-year honest-holdout diagnostic and validated deployed cap"
    )
...
def main() -> None:
    ...
    def cache_precompute_command(path_count: int, market_seed: int) -> str:
        """Return the exact authorised command for one required Q cache."""
        settings = q_cache_settings(path_count, market_seed)
        ...
        command = [
            "&",
            powershell_quote(Path(sys.executable).resolve()),
            powershell_quote(precompute),
            "--zero-curve",
            powershell_quote(Path(args.zero_curve).resolve()),
            "--model-parameters",
            powershell_quote(Path(args.model_parameters).resolve()),
            "--equity-allocation",
            powershell_quote(Path(equity_allocation.source_path).resolve()),
            "--market-cache-root",
            powershell_quote(Path(args.market_cache_root).resolve()),
            "--hedge-cache-root",
            powershell_quote(Path(args.hedge_cache_root).resolve()),
            "--horizon-years",
            f"{horizon_years:.12g}",
            "--n-paths",
            str(path_count),
    ...
    honest_fold_zero_path_count = int(sum(
        int(row["holdout_path_count"])
        for row in backward.first_year_action_rows
    ))
    lsmc_selection_path_count = int(sum(
    ...
    summary = {
        ...
        "method": (
            "honest fold-zero gas-storage LSMC with selection-only scaling, "
            "regression and inventory-grid fitting; statistical dynamic "
        ...
        "masked_selection_pool_bellman_first_year_cap_diagnostic": (
            backward.first_year_cap
        ),
        ...
        "bellman_value_diagnostic_sample_role": "honest_fold_zero",
        ...
        "n_lsmc_selection_paths": lsmc_selection_path_count,
        "n_honest_fold_zero_paths": honest_fold_zero_path_count,
        ...
        "cross_fit_folds_role": (
            "fold zero is excluded from all LSMC fits; remaining paths form "
            "the selection pool and use internal folds only for diagnostics"
        ),
