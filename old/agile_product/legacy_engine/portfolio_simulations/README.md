...
Vertrag ab t=0 und optimiert Income Election und den freiwilligen vollständigen
Exit in einer geordneten jährlichen Bellman-Rekursion:
...
monatliche Zahlung. Partial Withdrawal ist aus dem produktiven optimalen
Action Set ausgeschlossen. Materiell fehlende oder instabile Regressionen
machen den V11-Kandidaten ungültig; sie werden nicht still als WAIT oder
CONTINUE interpretiert. Die finale Bewertung verwendet in diesem Fall den auf
den unabhängigen Validierungspfaden aufgezeichneten festen Benchmark.

Growth-Surrender und Growth-Withdrawals bleiben vertraglich verboten.

Der Defaultlauf verwendet das vorab festgelegte primäre Trainings-Seed-Tripel
mit 20.000 Trainingspfaden, dazu 2.000 unabhängige Validierungspfade und weitere
2.000 davon getrennte finale Evaluationspfade. Die erhöhte Trainingsstichprobe
stabilisiert insbesondere die gepoolten Regressionen in späten, dünn besetzten
Income-Jahren. Replizierte Trainings-Seeds sind explizit zuschaltbar:
...
Trainingssample eingefrorene gemeinsame V11-Kandidatenpolicy wird im monatlichen
Projektor zuerst validiert. Election-only, Income-action-only und Combined
müssen jeweils eine gepaarte 95%-Nichtunterlegenheitsgrenze von minus einem
Basispunkt der Prämie gegen die vorab festgelegte Benchmarkbibliothek einhalten.
Besteht der Kandidat alle Gates, wird V11 final bewertet. Ist ein materieller
Regressionsfit oder ein Gate ungültig, bleibt V11 ausdrücklich ungültig und der
im Combined-Gate bereits ausgewiesene feste Benchmark wird stattdessen final
bewertet. Kandidatenstatus, deployte Policy und Fallback-Grund werden je Seed
in CSV und Manifest dokumentiert. Die Validation verändert oder refittet keine
Policy; das finale Sample wird niemals zur Policy-Auswahl verwendet.

Election und der freiwillige vollständige Income-Exit werden auf den jeweils
zulässigen vertraglichen Anniversaries entschieden. Separate Training-, Validation-
und Evaluation-Seeds für Marktpfade, Take-up und Mortalität werden im Manifest
und durch unterschiedliche Szenario-Fingerprints ausgewiesen. Für jeden Fit
werden Election-, Income- und Combined-Gate einzeln berichtet. Jeder Seed wird
auf denselben finalen Pfaden entweder mit dem bestandenen V11-Kandidaten oder
mit seinem aufgezeichneten festen Validierungsbenchmark bewertet. Seed 1 ist
vorab als Primary Policy festgelegt; die Evaluation wird nicht zur Auswahl
zwischen Seeds oder zum Wechsel zwischen Kandidat und Fallback verwendet.
...
unter deterministischer Election separat neu zu fitten. Die Election-Kontrolle
verwendet V10, sofern dieser Teilfit gültig ist, und andernfalls den festen V00-
Benchmark. Sie unterdrückt alle freiwilligen Income-Aktionen in der Evaluation,
ohne eine Continue-only-Election-Policy neu zu optimieren.
...
Der Risiko-Runner erzwingt für alle Bewertungs-Child-Runs den exakt passenden
Q-Marktcache und bei `mc_conditional` zusätzlich den exakt passenden
Hedgepreis-Cache. Vor der Bewertung ruft er automatisch
`precompute_q_market_and_hedge_cache.py` für Evaluation, jedes aktive
LSMC-Trainingssample und Validation sowie für jede benötigte Kombination aus
Marktstress und Single-Cap-Grid auf. Der Precompute-Runner validiert vorhandene
Einträge und erzeugt fehlende. Er bleibt die einzige schreibende Instanz; die
Bewertungsrunner selbst simulieren oder bepreisen weiterhin nicht ad hoc.
