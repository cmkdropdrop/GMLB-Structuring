...
from policy_engine.esg import Measure, ScenarioSet
from policy_engine.product import Index
from policy_engine.profitability import _irr
...
def test_profitability_accepts_default_valuation_reuse_but_rejects_policy_hook():
    class TaggedNeverSurrender:
        provenance_fingerprint = "external-policy-hook"
    ...
    settings = ValuationSettings(
        model="black_scholes",
        real_world_model="black_scholes",
        n_paths=12,
        seed=17,
        horizon_years=2.0,
        projection=ProjectionConfig(record_paths=False, max_age=67.0),
    )
    profitability_settings = ProfitabilitySettings(include_capital=False)
    common = dict(
        product=product,
        policy=policy,
        esg_config=cfg,
    ...
    default_valuation = value_contract(**common)
    reused = analyse_profitability(
        **common,
        prof_settings=profitability_settings,
        compute_capital_if_missing=False,
        valuation_result=default_valuation,
    )
    assert np.isfinite(reused.vnb_market_consistent)

    controlled_valuation = value_contract(
        **common,
        surrender_policy=TaggedNeverSurrender(),
    )
    with pytest.raises(
        ValueError,
        match="different assumptions or scenarios",
    ):
        analyse_profitability(
            **common,
            prof_settings=profitability_settings,
            compute_capital_if_missing=False,
            valuation_result=controlled_valuation,
        )
...
def test_generic_product_rejects_a_commencement_bonus():
    policy = PolicySpec(initial_investment=100_000.0, bonus_interest_pct=0.02,
                        income_start_year=1)
    with pytest.raises(ValueError, match="no commencement bonus"):
