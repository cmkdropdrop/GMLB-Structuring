...
from policy_engine.product import Phase
...
def test_array_kernel_preserves_exact_minimum_after_fraction_round_trip():
    product = IndexLinkedLifetimeIncomeProduct(
        mva=MVASpec(cost_loading=0.10),
    )
    inputs = _inputs(horizon=2.0, product=product)
    ...
    state = collector.states_by_step[13]
    scenario_slice = collector.slices_by_step[13]
    maximum = state.account_value - product.withdrawals.min_residual_value
    decision = IncomeActionDecision(
        np.full(
            state.n_paths,
        ...
        product.withdrawals.min_withdrawal / maximum,
    )

    transition = apply_income_action(
        state, decision, product, scenario_slice
    )

    np.testing.assert_allclose(
        transition.partial_withdrawal_cashflow
        + transition.mva_retained_cashflow,
        product.withdrawals.min_withdrawal,
        atol=1.0e-10,
    )
