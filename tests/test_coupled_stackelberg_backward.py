...
from policy_engine.optimal_behaviour_lsmc import OptimalBehaviourLSMCSettings
from policy_engine.product import Phase, PolicySpec
from policy_engine.projection import SurrenderDecisionContext
from policy_engine.stackelberg_control import (
    PolicyholderAction,
...
def _customer_core(
    ...
    pre = np.zeros((n_paths, len(_FOLLOWER_CORE_NAMES)), dtype=float)
    ...
    pre[:, FOLLOWER_CORE_INDEX["mva_factor"]] = rng.uniform(
        0.0, 0.10, n_paths
    )
    pre[:, FOLLOWER_CORE_INDEX["attained_age"]] = rng.uniform(
        65.0, 90.0, n_paths
    )
    pre[:, FOLLOWER_CORE_INDEX["primary_alive"]] = 1.0
    pre[:, FOLLOWER_CORE_INDEX["spouse_alive"]] = 0.0
    pre[:, FOLLOWER_CORE_INDEX["inforce_weight"]] = 1.0
...
def test_fold_local_instability_does_not_discard_stable_full_sample_policy():
    fixture = _finite_state_fixture(policyholder_actions_enabled=True)
    # 60 observations are enough for the full canonical fit (minimum 48),
    # while every 3-fold outer training slice stays below 48 and must
    ...
    for record in fixture.data.signature_control_paths[
        ...
        inforce_index = FOLLOWER_CORE_INDEX["inforce_weight"]
        record.after_cap_core[:, inforce_index] = 0.0
        record.after_cap_core[eligible_indices, inforce_index] = 1.0
