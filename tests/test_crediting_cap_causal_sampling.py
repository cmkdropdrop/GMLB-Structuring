...
from policy_engine import (ESGConfig, Index, Measure, MortalityTable,
                           ProjectionConfig, ScenarioSet, YieldCurve)
...
def test_fixed_lsmc_fits_only_training_and_selects_without_final_paths(
    ...
    projection_calls: list[
        tuple[object, np.ndarray, float | None, object]
    ] = []
    ...
    def fake_aggregate(**kwargs):
        ...
        cap_matrix = np.asarray(kwargs["cap_matrix"], dtype=float)
        cap = float(cap_matrix[0, 0])
        factory = kwargs.get("combined_policy_factory")
    ...
    (
        ...
        expenses=object(),
        projection_config=ProjectionConfig(record_paths=False),
        portfolio_scale=1.0,
    ...
    assert len(projection_calls) == 3 * len(cases)
    assert all(
        sample is evaluation
        for sample, _caps, _fit_cap, _mortality in projection_calls
    )
    assert sum(
        fitted_cap is not None
        for _, _, fitted_cap, _mortality in projection_calls
    ) == 2 * len(cases)
    mortality_curves = [
        mortality_basis.monthly_q_curve(65.0, "M", 12)
        for _, _, _fitted_cap, mortality_basis in projection_calls
    ]
    assert sum(np.all(curve == 0.0) for curve in mortality_curves) == (
        2 * len(cases)
    )
    ...
    assert all(
        ...
        and row["policyholder_mortality_basis"]
        == optimizer.POLICYHOLDER_LSMC_MORTALITY_BASIS
        and row["death_termination_in_policyholder_validation"] is False
        and row["only_voluntary_termination_action"]
        == "FULL_WITHDRAWAL_NOW"
...
def test_projected_state_rollout_freezes_actions_from_time_prefixes(monkeypatch):
    ...
    def fake_portfolio_extension(projected):
        n_state_years = projected.cap_matrix.shape[1] + 1
        n_features = len(optimizer.PORTFOLIO_CONTROL_STATE_FEATURE_NAMES)
        rich = np.zeros(
            (projected.scenarios.n_paths, n_state_years, n_features),
            dtype=float,
        )
        for policy_year in range(n_state_years):
            decision_step = min(
                policy_year * 12, projected.scenarios.n_steps
            )
            marker = np.asarray(
    ...
    assert [call.scenarios.n_steps for call in baseline_calls] == [12, 12, 24, 36]
    assert [call.scenarios.n_steps for call in changed_calls] == [12, 12, 24, 36]
