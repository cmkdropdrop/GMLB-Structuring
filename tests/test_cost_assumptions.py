...
def _static_behaviour(
    growth_lapse: float = 0.0,
    income_lapse: float = 0.0,
) -> BehaviourModel:
    return BehaviourModel(
        regime="static",
        lapse=LapseAssumptions(
            growth_phase=(growth_lapse,), income_phase=income_lapse
        ),
...
def test_base_mva_loading_is_retained_and_increases_insurer_nav():
    ...
    # The active generic product prohibits surrender during Growth.  Exercise
    # the MVA through an Income-phase full-withdrawal decrement after the first
    # Anniversary election instead of relying on obsolete source-product
    # Growth-lapse mechanics.  A two-year horizon is required because lapse is
    # suppressed at the election timestamp itself.
    behaviour = _static_behaviour(income_lapse=0.25)
