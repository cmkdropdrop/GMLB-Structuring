...
1. Age Pension+ cap is applied to excess withdrawals *before* the MVA, so the
   net payout can never go negative (previously the MVA was computed on the
   uncapped amount).
...
class TestApsMvaOrdering:
    """Fix 1: APS-capped excess withdrawals cannot pay out negative amounts."""

    def test_no_negative_partial_withdrawals(self):
        cfg = ESGConfig(curve=STEEP_CURVE)   # forwards > spot => MVA bites
        policy = PolicySpec(age=65, income_start_year=1, age_pension_plus=True)
