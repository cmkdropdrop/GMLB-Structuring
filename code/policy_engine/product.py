...
``documents/product_design.md``.  It has one
...
Some source-product enums and rate-card columns remain as compatibility scaffolding
...
#: Legacy source-product Maximum Returns for July 2026 (compatibility only).
...
class MVASpec:
    ...
    the public source-product illustration. Both remain calibration inputs: the
    disclosure does not reveal any insurer's production MVA formula. The neutral constructor keeps
    ...
    def pds_illustrative_termination_loading(cls, **kwargs) -> "MVASpec":
        ...
        it is not an insurer's undisclosed production MVA calibration.
        """
...
class PolicySpec:
    ...
    #: Legacy source-product allocation retained only for input compatibility. The
