...
def apply_income_action(
    ...
    *,
    ...
    # A policy specifies the amount as a fraction of the pathwise maximum;
    # tolerate only machine-roundoff when that round trip represents the
    # contractual minimum exactly.
    minimum_withdrawal = float(product.withdrawals.min_withdrawal)
    minimum_tolerance = (
        32.0 * np.finfo(float).eps * max(abs(minimum_withdrawal), 1.0)
    )
    partial = requested_partial & (
        gross_request >= minimum_withdrawal - minimum_tolerance
    )
    ...
    full = requested_full
    gross = np.where(
        partial, np.maximum(gross_request, minimum_withdrawal), 0.0
    )
...
class SurrenderDecisionContext:
    ...
    :class:`~policy_engine.esg.ScenarioSet`.  Every path array is defensively
...
class ProjectionResult:
    ...
    def identity_gap(self) -> float:
        ...
        Returns the relative gap.  It should be near zero when the customer
        package is valued exactly under the scenario model.  For the generic
        mixed fund, the documented ``moment_matched_bs`` DVA proxy can leave a
        model-basis residual in addition to Monte-Carlo and discretisation
        error; the diagnostic reports that residual rather than concealing it.
        """
        pv = self.pv_by_component()
...
def project(product: IndexLinkedLifetimeIncomeProduct, policy: PolicySpec,
    ...
    if config.hedge_pricing_method == "mc_conditional":
        ...
        hedge_spec = hedge_price_surface.spec
        hedge_mismatches: list[str] = []
        if hedge_spec.equity_index != reference_spec.equity_index:
            hedge_mismatches.append("equity_index")
        if not np.isclose(
            hedge_spec.equity_allocation,
            reference_spec.equity_weight,
            rtol=0.0,
            atol=1.0e-15,
        ):
            hedge_mismatches.append("equity_allocation")
        if not np.isclose(
            hedge_spec.bond_tenor_years,
            reference_spec.bond_tenor_years,
            rtol=0.0,
            atol=1.0e-12,
        ):
            hedge_mismatches.append("bond_tenor_years")
        if (
            hedge_spec.monthly_rebalancing is not True
            or reference_spec.rebalance_frequency_months != 1
        ):
            hedge_mismatches.append("monthly_rebalancing")

        required_hedge_starts = np.arange(
            0, n_steps, STEPS_PER_YEAR, dtype=np.int64,
        )
        cached_hedge_starts = np.asarray(
            hedge_price_surface.annual_start_steps, dtype=np.int64,
        )
        if not np.all(np.isin(required_hedge_starts, cached_hedge_starts)):
            hedge_mismatches.append("annual_start_steps")
        required_caps = np.asarray([
            reference_spec.cap(int(step // STEPS_PER_YEAR))
            for step in required_hedge_starts
        ], dtype=float)
        cached_caps = np.asarray(hedge_price_surface.cap_grid, dtype=float)
        # A controlled optimisation may supply one cap per path and year,
        # whereas a regular valuation supplies one scalar per year.  Compare
        # the distinct requested actions with the cached grid so both shapes
        # follow the same strict membership rule without accidental NumPy
        # broadcasting between ``(n_caps,)`` and ``(n_paths,)``.
        distinct_required_caps = np.unique(required_caps)
        if any(
            not np.any(np.isclose(
                cached_caps, cap, rtol=0.0, atol=1.0e-12,
            ))
            for cap in distinct_required_caps
        ):
            hedge_mismatches.append("cap_grid")
        if hedge_mismatches:
            raise ValueError(
                "Bound mc_conditional HedgePriceSurface conflicts with the "
                "product reference fund: " + ", ".join(hedge_mismatches) + "."
            )

        # This array is part of the validated, path-aligned hedge artefact.
        # Re-deriving it here could silently diverge from the paths on which
        # the cached conditional option prices were fitted.
        reference_fund_level = hedge_price_surface.reference_fund_index[
            :, :n_steps + 1
        ]
    else:
    ...
    def choose_income_action(
        ...
        # not a smaller Partial Withdrawal.  The amount is supplied as a
        # fraction of the pathwise maximum, so tolerate only machine-roundoff
        # when that round trip represents the contractual minimum exactly.
        minimum_withdrawal = float(product.withdrawals.min_withdrawal)
        minimum_tolerance = (
            32.0 * np.finfo(float).eps * max(abs(minimum_withdrawal), 1.0)
        )
        partial = requested_partial & (
            gross >= minimum_withdrawal - minimum_tolerance
        )
        gross = np.where(
            partial, np.maximum(gross, minimum_withdrawal), 0.0
        )
