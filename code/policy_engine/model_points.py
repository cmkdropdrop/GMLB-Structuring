...
The operational default is the fast four-point proxy; the legacy source-shaped
...
from .repository_paths import DEFAULT_POLICYHOLDER_MODEL_POINTS_PATH
    ...
    *,
    ...
    for row_number, row in enumerate(reader, start=2):
        ...
        if any(weight < 0.0 for weight in legacy_allocation) or not isclose(
            ...
            raise ValueError(
                f"Legacy allocation must be non-negative and sum to one at row {row_number}."
            )
