...
Growth ``WAIT``/``START_INCOME_NOW`` and annual Income
``CONTINUE_FOR_ONE_YEAR``/``FULL_WITHDRAWAL_NOW`` actions.  Partial withdrawal
is deliberately outside the action set used by this delegated optimiser.

The delegated optimiser uses its development defaults: the four-point proxy,
a coarse cap grid, three fresh cap-specific finalist fits, and no separate Dynamic or
